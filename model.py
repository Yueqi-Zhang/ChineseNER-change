...
from data_utils import create_input, iobes_iob, seperate_input, get_batch
...
class Model(object):
    def __init__(self, config):
        ...
        self.num_sents = tf.shape(self.char_inputs)[0]

        self.sequence_length = tf.placeholder(dtype=tf.int32, shape=[None], name="SequenceLength")
        ...
        lstm_outputs = self.biLSTM_layer(lstm_inputs, self.sequence_length)
    ...
    def biLSTM_layer(self, lstm_inputs, sequence_length):
        """
    ...
    def loss_layer(self, project_logits):
        ...
        with tf.variable_scope("crf_loss"):
            ...
            targets = tf.concat(
                [tf.ones([tf.shape(self.targets)[0], 1], tf.int32) * self.num_tags, self.targets, tf.ones([tf.shape(self.targets)[0], 1], tf.int32) * tf.add(self.num_tags,1)], axis=-1)
    ...
    def create_feed_dict(self, is_train, batch):
        ...
        """
        chars, segs, tags, sequence_length = batch
        feed_dict = {
            self.char_inputs: np.asarray(chars),
            self.seg_inputs: np.asarray(segs),
            self.targets: np.asarray(tags),
            self.dropout: 1.0,
            self.sequence_length: np.asarray(sequence_length)
        }
        if is_train:
    ...
    def run_step(self, sess, is_train, batch):
        ...
        if is_train:
            ...
            lengths, logits, loss = sess.run([self.num_steps, self.logits, self.loss], feed_dict)
